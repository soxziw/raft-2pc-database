...
AIOServer::AIOServer(int cluster_id, int server_id,
    ...
    // Init
    std::printf("(%d)[%d:%d][INIT] Init async I/O server.\n", getpid(), cluster_id, server_id);
...
void AIOServer::broadcast_vote() {
    ...
    std::printf("(%d)[%d:%d][CANDIDATE:%d] Vote.\n", getpid(), cluster_id_, server_id_, raft_state_->current_term_);
...
void AIOServer::broadcast_heart_beat() {
    std::printf("(%d)[%d:%d][LEADER:%d] Heart beat.\n", getpid(), cluster_id_, server_id_, raft_state_->current_term_);
    // Send to routing service
    WrapperMessage* wrapper_msg = new WrapperMessage;
    ...
    // Broad cast to other servers in the same cluster
    raft_state_->coming_commit_index_ = raft_state_->log_.size() - 1; // Set coming commit index
    for (int idx = 0; idx < SERVER_NUM_PER_CLUSTER; idx++) {
        if (idx == server_id_ % SERVER_NUM_PER_CLUSTER) {
            continue;
        }
        ...
        // Put log in [next_log_index_,log.size()) into heart beat message
...
int AIOServer::setup_listening_socket(int port) {
    std::printf("(%d)[%d:%d] Listen on port %d.\n", getpid(), cluster_id_, server_id_, port);
    int sock;
...
void AIOServer::run(int server_socket) {
    std::printf("(%d)[%d:%d] Start running.\n", getpid(), cluster_id_, server_id_);
    ...
    while (keep_running_) {
        ...
        switch (data->event_type) {
            case AIOEventType::EVENT_TIMEOUT: { // Timeout event
                std::printf("(%d)[%d:%d] Timeout event.\n", getpid(), cluster_id_, server_id_); 
            ...
            case AIOEventType::EVENT_ACCEPT: { // Accept event
                std::printf("(%d)[%d:%d] Accept event from socket %d.\n", getpid(), cluster_id_, server_id_, cqe->res);
            ...
            case AIOEventType::EVENT_CONNECT: { // Connect event
                std::printf("(%d)[%d:%d] Connect event to socket %d.\n", getpid(), cluster_id_, server_id_, data->fd);
                // Write
            ...
            case AIOEventType::EVENT_READ: {
                std::printf("(%d)[%d:%d] Read event from socket %d, message: %s.\n", getpid(), cluster_id_, server_id_, data->fd, data->buf);
            ...
            case AIOEventType::EVENT_WRITE: {
                std::printf("(%d)[%d:%d] Write event to socket %d.\n", getpid(), cluster_id_, server_id_, data->fd);
...
void AIOServer::process(int client_socket, const WrapperMessage* wrapper_msg) {
    if (wrapper_msg->message_type_case() == WrapperMessage::kExit) { // Exit message
        std::printf("(%d)[%d:%d] Exit message from socket %d.\n", getpid(), cluster_id_, server_id_, client_socket);
    ...
    if (wrapper_msg->message_type_case() == WrapperMessage::kResume) { // Resume message
        std::printf("(%d)[%d:%d] Resume message from socket %d.\n", getpid(), cluster_id_, server_id_, client_socket);
    ...
    if (wrapper_msg->message_type_case() == WrapperMessage::kStop) { // Stop message
        std::printf("(%d)[%d:%d] Stop message from socket %d.\n", getpid(), cluster_id_, server_id_, client_socket);
    ...
    switch (wrapper_msg->message_type_case()) {
        case WrapperMessage::kIntraShardReq: { // Intra-shard request
            std::printf("(%d)[%d:%d] Intra-shard request from socket %d.\n", getpid(), cluster_id_, server_id_, client_socket);
        ...
        case WrapperMessage::kCrossShardReq: {
            std::printf("(%d)[%d:%d] Cross-shard request from socket %d.\n", getpid(), cluster_id_, server_id_, client_socket);
        ...
        case WrapperMessage::kAppendEntriesReq: {
            std::printf("(%d)[%d:%d] Append entries request from socket %d.\n", getpid(), cluster_id_, server_id_, client_socket);
        ...
        case WrapperMessage::kAppendEntriesRsp: {
            std::printf("(%d)[%d:%d] Append entries response from socket %d.\n", getpid(), cluster_id_, server_id_, client_socket);
        ...
        case WrapperMessage::kRequestVoteReq: {
            std::printf("(%d)[%d:%d] Request vote request from socket %d.\n", getpid(), cluster_id_, server_id_, client_socket);
        ...
        case WrapperMessage::kRequestVoteRsp: {
            std::printf("(%d)[%d:%d] Request vote response from socket %d.\n", getpid(), cluster_id_, server_id_, client_socket);
        ...
        case WrapperMessage::kPrintBalanceReq: {
            std::printf("(%d)[%d:%d] Print balance request from socket %d.\n", getpid(), cluster_id_, server_id_, client_socket);
        ...
        case WrapperMessage::kPrintDatastoreReq: {
            std::printf("(%d)[%d:%d] Print datastore request from socket %d.\n", getpid(), cluster_id_, server_id_, client_socket);
